...
public class VHostCPUMonitor_DRS1 {
	...
	private void addVM() throws Exception {
		...
		int vHostCPU_141 = vHostMap.get(ConstantUtil.VHost1);
		int vHostCPU_142 = vHostMap.get(ConstantUtil.VHost2);
		ResourcePool rp = null;
		String targetHostName;

		String dcName = "DC_Team05";
		...
		vmSpec.setName(ConstantUtil.NEW_VM_NAME);
		vmSpec.setAnnotation(ConstantUtil.NEW_VM_ANNOTATION);
		vmSpec.setMemoryMB(memorySizeMB);
		vmSpec.setNumCPUs(cupCount);
		vmSpec.setGuestId(ConstantUtil.GuestOS);

		// create vm file info for the vmx file
		VirtualMachineFileInfo vmfi = new VirtualMachineFileInfo();
		vmfi.setVmPathName("[" + ConstantUtil.NFS_Name + "]");
		 ...
		 * VirtualMachineRelocateSpec vmRelocate = new
		 * VirtualMachineRelocateSpec(); vmRelocate.setHost(targetHost);
		 * cloneSpec.setLocation(vmRelocate); cloneSpec.setPowerOn(false);
		...
		VirtualMachine newVM = (VirtualMachine) new InventoryNavigator(
				serviceInstance.getRootFolder()).searchManagedEntity("VirtualMachine", ConstantUtil.NEW_VM_NAME);
		Task tempTask = newVM.powerOnVM_Task(null);
		tempTask.waitForMe();
