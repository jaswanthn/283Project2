...
public class VHostCPUMonitor_DRS2 {
	...
	public void addVHost() {
		try {
			...
			hcs.setHostName("130.65.132.173");
			hcs.setPassword("12!@qwQW");
			hcs.setUserName("root");
			hcs.setSslThumbprint(ConstantUtil.SSL_THUMBPRINT_HOST_173);

			// vcenter ip
			hcs.setManagementIp("130.65.132.170");
	...
	private String getMinUsageVM(HostSystem host) throws Exception {
		...
		if (host.getVms() != null && host.getVms().length > 0) {
			for (VirtualMachine vmObj : host.getVms()) {
				...
				for (String metricNam : ConstantUtil.METRIC_LIST) {
					...
					for (String p : metricProps.keySet()) {
						if (ConstantUtil.PROJECT_PARAMETER_LIST_DRS2
								.contains(p)) {
							if (vmStat == -1) {
		...
		System.out.println("vHost is -->" + host.getName()
				+ " VM to be migrated is-->" + minUsageVM.getName());
	...
	/**
	 * This method will generate the Performance Results based on the
	 * performance metrics
	 * 
	 * @param pValues
	 * @return
