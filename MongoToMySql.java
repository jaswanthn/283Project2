...
public class MongoToMySql {
	...
	public MongoToMySql() {
		...
		try {
			...
			connect = (Connection) DriverManager
					.getConnection("jdbc:mysql://localhost/CMPE283PROJ2?"
							+ "user=root&password=");
			metadata = connect.getMetaData();
			statement = (Statement) connect.createStatement();
			// mongo = new MongoClient("130.65.251.52", 27017);
	...
	private void analyseAndPushToSQL(String mysqltableName) throws Exception {
		...
		if (recordCount > 0) {
			if (mysqltableName.startsWith("HOST"))
				preparedStatement = (PreparedStatement) connect
						.prepareStatement(ConstantUtil.PREPARED_STATEMENT_START
								+ mysqltableName + ConstantUtil.PREPARED_STATEMENT_LAST);
			...
			preparedStatement.setString(15, timestamp);
	...
	public boolean createTable(String name) throws SQLException {
		...
		try {
			...
			// System.out.println(ConstantUtil.CREATE_TABLE_START + name
			// + ConstantUtil.CREATE_VM_TABLE_LAST);
			preparedStatement.execute();
			successStatus = true;
		} catch (Exception e) {
